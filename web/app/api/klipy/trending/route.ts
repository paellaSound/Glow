...
export async function GET(req: NextRequest) {
  ...
  try {
    const url = `https://api.klipy.com/api/v1/${appKey}/gifs/trending?page=${page}&per_page=${perPage}&customer_id=${customerId}&content_filter=safe`;
    const res = await fetch(url);
    if (!res.ok) {
      return NextResponse.json({ error: 'Failed to fetch trending GIFs from Klipy' }, { status: res.status });
    }
